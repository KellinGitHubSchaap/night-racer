...
//
//
// Deprecated script! Will be removed in next release. Currently left only for addon prefabs which's demo scenes are using this script.
//
//
public class MobileSetRPMToSlider : MonoBehaviour { // this script was only made for slider demo scene for demonstration purposes only
    ...
    private void Start()
    {
        ...
        res_mob.carCurrentSpeed = (int)rpmSlider.value;
        res_mob.carMaxSpeed = res_mob.maxRPMLimit;
    }
    public void SetRPM()
    {
        if (res_mob != null)
